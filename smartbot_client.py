...
def send_message(
    ...
    if web_url:
        text = (
            f"✨ Ура, приключение готово!\n\n"
            f"Ваш персональный лист с заданиями ждет по ссылке:\n"
            f"👉 {web_url}\n\n"
            f"🖨 Как получить бумажную версию:\n"
            f"Просто откройте ссылку в браузере и нажмите кнопку «Печать» (или Ctrl+P).\n\n"
            f"Желаем ребенку отличной игры и легкой учебы! ❤️\n\n"
            f"🤖 Сгенерировано нейросетью в @KidionAIBot.\n"
            f"Проверьте перед использованием."
        )
